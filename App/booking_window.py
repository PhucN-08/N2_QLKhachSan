...
def parse_and_validate_dates(checkin_text, checkout_text):
    try:
        checkin_d = datetime.strptime(checkin_text.strip(), "%d/%m/%Y").date()
    except ValueError:
        return False, "Ngày check-in không đúng định dạng (dd/mm/yyyy).", None, None
    try:
        checkout_d = datetime.strptime(checkout_text.strip(), "%d/%m/%Y").date()
    except ValueError:
...
def add_booking(customer_id, room_id, checkin_date, checkout_date):
    ok, msg, checkin_d, checkout_d = parse_and_validate_dates(checkin_date, checkout_date)
    if not ok:
        return False, msg
    ...
    try:
        ...
        cursor.execute(
            ...
            (customer_id, room_id, checkin_d, checkout_d)
        )
    ...
    try:
        ...
        cursor.execute(
            """UPDATE bookings SET checkin_date = %s, checkout_date = %s
               WHERE id = %s""",
            (checkin_d, checkout_d, booking_id)
        )
        conn.commit()
...
def cancel_booking(booking_id):
    ...
    try:
        ...
        cursor.execute("UPDATE rooms SET status = 'Empty' WHERE id = %s", (room_id,))
        conn.commit()
...
def checkin(booking_id):
    ...
    try:
        ...
        room_id = row[0]
        cursor.execute(
            "UPDATE bookings SET actual_checkin = %s, status = 'CheckedIn' WHERE id = %s",
...
def checkout(booking_id):
    ...
    try:
        ...
        room_id = row[0]
        cursor.execute(
            "UPDATE bookings SET actual_checkout = %s, status = 'CheckedOut' WHERE id = %s",
            (datetime.now(), booking_id)
        )
        cursor.execute("UPDATE rooms SET status = 'Empty' WHERE id = %s", (room_id,))
        conn.commit()
...
def change_room(booking_id, new_room_id):
    ...
    try:
        ...
        cursor.execute("SELECT status FROM rooms WHERE id = %s", (new_room_id,))
        ...
        cursor.execute("UPDATE bookings SET room_id = %s WHERE id = %s", (new_room_id, booking_id))
        new_room_status = "Occupied" if status == "CheckedIn" else "Booked"
        cursor.execute("UPDATE rooms SET status = %s WHERE id = %s", (new_room_status, new_room_id))
        cursor.execute("UPDATE rooms SET status = 'Empty' WHERE id = %s", (old_room_id,))
        conn.commit()
...
def get_available_rooms():
    conn = get_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT id, room_number, room_type FROM rooms WHERE status = 'Empty' ORDER BY room_number")
    result = cursor.fetchall()
    cursor.close()
    conn.close()
    return result
...
class BookingWindow(tk.Frame):
    ...
    def __init__(self, master, *args, **kwargs):
        ...
        self._add_placeholder(self.entry_checkin, "dd/mm/yyyy")
        self._add_placeholder(self.entry_checkout, "dd/mm/yyyy")
    ...
    def _build_header(self):
        ...
        tk.Label(header, text="QUẢN LÝ ĐẶT PHÒNG", font=FONT_TITLE,
    ...
    def _build_form_card(self):
        ...
        tk.Label(card, text="Thông tin đặt phòng", font=FONT_HEADING,
        ...
        self._field_label(form, "Khách hàng", 0, 0)
        self.cb_customer = ttk.Combobox(form, state="readonly", font=FONT_BODY)
        self.cb_customer.grid(row=0, column=1, sticky="ew", padx=(0, 20), pady=6)

        self._field_label(form, "Phòng", 0, 2)
        self.cb_room = ttk.Combobox(form, state="readonly", font=FONT_BODY)
        self.cb_room.grid(row=0, column=3, sticky="ew", pady=6)

        # Hàng 2: Ngày check-in - check-out
        self._field_label(form, "Check-in", 1, 0)
        self.entry_checkin = tk.Entry(form, font=FONT_BODY, relief="solid",
                                       borderwidth=1, highlightthickness=0)
        self.entry_checkin.grid(row=1, column=1, sticky="ew", padx=(0, 20), pady=6, ipady=3)

        self._field_label(form, "Check-out", 1, 2)
        self.entry_checkout = tk.Entry(form, font=FONT_BODY, relief="solid",
                                        borderwidth=1, highlightthickness=0)
        self.entry_checkout.grid(row=1, column=3, sticky="ew", pady=6, ipady=3)

        # Hàng 3: Phòng mới (đổi phòng)
        self._field_label(form, "Phòng mới (đổi phòng)", 2, 0)
        self.cb_new_room = ttk.Combobox(form, state="readonly", font=FONT_BODY)
        self.cb_new_room.grid(row=2, column=1, sticky="ew", padx=(0, 20), pady=(6, 12))
        ...
        ttk.Button(toolbar, text="+ Thêm đặt phòng", style="Add.TButton",
                   command=self.on_add).pack(side="left", padx=(0, 8))
        ttk.Button(toolbar, text="Sửa", style="Edit.TButton",
                   command=self.on_edit).pack(side="left", padx=8)
        ttk.Button(toolbar, text="Hủy đặt phòng", style="Cancel.TButton",
                   command=self.on_cancel).pack(side="left", padx=8)
        ttk.Button(toolbar, text="Check-in", style="CheckIn.TButton",
                   command=self.on_checkin).pack(side="left", padx=8)
        ttk.Button(toolbar, text="Check-out", style="CheckOut.TButton",
                   command=self.on_checkout).pack(side="left", padx=8)
        ttk.Button(toolbar, text="Đổi phòng", style="ChangeRoom.TButton",
                   command=self.on_change_room).pack(side="left", padx=8)
    ...
    def _build_table(self):
        ...
        columns = ("id", "customer", "room", "checkin", "checkout",
                   "actual_checkin", "actual_checkout", "status", "booking_date")
        headers = {
            "id": "Mã ĐP", "customer": "Khách hàng", "room": "Phòng",
            "checkin": "Check-in dự kiến", "checkout": "Check-out dự kiến",
            "actual_checkin": "Check-in thực tế", "actual_checkout": "Check-out thực tế",
            "status": "Trạng thái", "booking_date": "Ngày đặt"
        }
        widths = {
            "id": 60, "customer": 150, "room": 130, "checkin": 120,
            "checkout": 120, "actual_checkin": 140, "actual_checkout": 140,
            "status": 120, "booking_date": 140
        }
        ...
        for status, color in STATUS_COLORS.items():
            self.tree.tag_configure(status, foreground=color, font=("Segoe UI", 10, "bold"))
    ...
    def _build_status_bar(self):
        ...
        self.lbl_count = tk.Label(bar, text="0 đặt phòng", font=FONT_SMALL,
    ...
    def load_comboboxes(self):
        try:
            ...
            self.customers_map = {f"{c['id']}-{c['name']}": c["id"] for c in customers}
            self.cb_customer["values"] = list(self.customers_map.keys())

            all_rooms = get_all_rooms()
            self.rooms_map = {f"{r['room_number']} - {r['room_type']}": r["id"] for r in all_rooms}
            self.cb_room["values"] = list(self.rooms_map.keys())
            self.cb_new_room["values"] = list(self.rooms_map.keys())
        except Error as e:
    ...
    def load_bookings(self):
        try:
            ...
            messagebox.showerror("Lỗi", f"Không thể tải danh sách đặt phòng: {e}")
            self.all_bookings = []
    ...
    def render_rows(self, bookings):
        ...
        for i, b in enumerate(bookings):
            ...
            self.tree.insert("", "end", iid=b["id"], tags=(stripe, b["status"]), values=(
                b["id"],
                b["customer_name"],
                ...
                STATUS_LABELS_VI.get(b["status"], b["status"]),
                format_date_vi(b["booking_date"])
            ))
        self.lbl_count.config(text=f"{len(bookings)} đặt phòng")
    ...
    def apply_filters(self):
        ...
        if keyword:
            filtered = [b for b in filtered if keyword in b["customer_name"].lower()]
        if status_label and status_label != "Tất cả":
            filtered = [b for b in filtered if STATUS_LABELS_VI.get(b["status"]) == status_label]
    ...
    def on_select_row(self, event):
        ...
        booking = next((b for b in self.all_bookings if b["id"] == booking_id), None)
        if not booking:
            return

        customer_key = f"{booking['customer_id']}-{booking['customer_name']}"
        room_key = f"{booking['room_number']} - {booking['room_type']}"
        ...
        self._set_date_field(self.entry_checkin, format_date_vi(booking["checkin_date"]))
        self._set_date_field(self.entry_checkout, format_date_vi(booking["checkout_date"]))

    def get_selected_id(self):
        selected = self.tree.focus()
        if not selected:
            messagebox.showwarning("Thông báo", "Vui lòng chọn một đặt phòng trong danh sách.")
            return None
        return int(selected)
    ...
    def on_reset(self):
        ...
        self._set_date_field(self.entry_checkin, "")
        self.entry_checkin.event_generate("<FocusOut>")
        self._set_date_field(self.entry_checkout, "")
        self.entry_checkout.event_generate("<FocusOut>")
        self.load_bookings()
    ...
    def on_add(self):
        ...
        if not customer_key or not room_key or not checkin_date or not checkout_date:
        ...
        ok, msg = add_booking(customer_id, room_id, checkin_date, checkout_date)
        self.set_status(msg)
        if ok:
    ...
    def on_edit(self):
        ...
        if not checkin_date or not checkout_date:
            messagebox.showwarning("Thông báo", "Vui lòng nhập ngày check-in/check-out.")
            return

        ok, msg = edit_booking(booking_id, checkin_date, checkout_date)
        self.set_status(msg)
        if ok:
            messagebox.showinfo("Thành công", msg)
            self.load_bookings()
    ...
    def on_cancel(self):
        ...
        if booking_id is None:
            return
        if not messagebox.askyesno("Xác nhận", "Bạn có chắc muốn hủy đặt phòng này?"):
            return
    ...
    def on_change_room(self):
        ...
        if not new_room_key:
            messagebox.showwarning("Thông báo", "Vui lòng chọn phòng mới.")
            return
        new_room_id = self.rooms_map[new_room_key]

        ok, msg = change_room(booking_id, new_room_id)
        self.set_status(msg)
        if ok:
            messagebox.showinfo("Thành công", msg)
            self.load_bookings()
